...
%   SUBPLOT2(H), where H is an axis handle, is another way of making
...
%   the statement SUBPLOT2(1,2,1) deletes all existing axes overlapping
%   the left side of the Figure window and creates a new axis on that
%   side.
%
%   SUBPLOT2(111) is an exception to the rules above, and is not
%   identical in behavior to SUBPLOT2(1,1,1).  For reasons of backwards
...
elseif(isempty(position))
    if (min(thisPlot) < 1)
        ...
        % PERC_OFFSET_L = 2*0.09; <= Original
        % PERC_OFFSET_R = 2*0.045; <= Original
        % PERC_OFFSET_B = PERC_OFFSET_L; <= Original
        % PERC_OFFSET_T = PERC_OFFSET_R; <= Original
        PERC_OFFSET_L = 2*0.06;     % <= Modified by Brian Schlining
        PERC_OFFSET_R = 2*0.03;     % <= Modified by Brian Schlining
        PERC_OFFSET_B = 2*0.09;     % <= Modified by Brian Schlining
        PERC_OFFSET_T = 2*0.045;    % <= Modified by Brian Schlining
