function [Y, a, r2, p] = regress_logprofile(z, y)
% REGRESS_HARMONIC  - fits data to sin Wt cos Wt + C 
%
% Use as: [Y, a, r, p] = regress_logprofile(z, y)
%         [Y, a, r, p] = regress_logprofile(z, y)
%
% Inputs: z = vector of depths of each sample 
...
[rz cz] = size(z);
if (rz > 1 & cz > 1)
   error('REGRESS_LOGPROFILE only accepts vectors as inputs')
elseif (cz > 1 & rz == 1)
   z = z';
end
...
if length(z) ~= length(y)
   error('Vectors z and y must be the same length');
end

% X = sin(Omega*T) + cos(Omega*t) + C
X = [log(z) ones(size(z))];  % Create matrix
...
%==================================================================
n = length(z);    % length of input vector
