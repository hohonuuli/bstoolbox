% wrldmap
% W. Broenkow added Mercator to WHOI version
...
if (maxlat>90); error('MAXLAT greater than 90!'); end;if (nargin>0),
  if (isstr(axes)),
    if (axes(1:3)=='nor'),
      proj = 'npl';
...
%elseif (proj=='mer'),
  mer = mercator(coastlines(2,:));
  lh  = plot(coastlines(1,:)+0,mer,'-');
  set(lh,'Erasemode','none');
pause
elseif (proj=='npl'),
...
if (drawtwice),
  line(coastlines(1,:),mer,'Linestyle','-',...
  'Erasemode','none');
end;
...
yx  = get(gca,'Ytick')
ypos = mercator(yx);           % get mercator values from rectangular
...
for kk=1:Nn,
  zz = str2num(ylb(kk,:));
  if (zz<0),
    nyl(kk,:) = sprintf(['%' Mf '.0fS'],abs(zz));
  elseif (zz>0),
    nyl(kk,:) = sprintf(['%' Mf '.0fN'],(zz)); 
  else
    nyl(kk,:) = sprintf(['%' Mf '.0f '],(zz));
  end;
end;
set(gca,'Yticklabels',nyl);
set(gca,'Ytick',yx);  %  reset position limits
...
%******************************************************
  ...
  for kk=0:30:359,
    ...
    line(xx,yy,'color','w','linestyle',':');
    if (kk<=180),
      nyl = sprintf('%3.0fE',kk);
    else 
      nyl = sprintf('%3.0fW',abs(360-kk));
    end;
    if (proj=='npl'), 
