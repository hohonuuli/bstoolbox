function y = readfmtfile(Filename, fmt)
% READFMTFILE - Read a file using a specified format
%
% Use As: out = readfmtfile(filename, fmt)
% Inputs: filename = the name fo the file to read
%         fmt = The format (e.g %f %f,%f,%f,%f,%f,%i)
...
if ~nargin | isempty(Filename)
   [infile inpath] = uigetfile('*.*','Select file',0,0);
   if infile == 0
      y = [];
